...
def strike_at_index(collection: list, strike_index: int, radius: int):
    ...
    if is_index_valid(collection, left_strike_index) and is_index_valid(collection, strike_index) and is_index_valid(collection, right_strike_index):
...
while command != "End":
    ...
    if cmd_type == "Shoot":
        ...
        strike_at_index(numbers, cmd_index, cmd_value)
    command = input()
